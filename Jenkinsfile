node() {
    ...
    stage('Import results to Xray') {
       ...
       "fields": {
        ...
        echo "${info}"
        step([$class: 'XrayImportBuilder', endpointName: '/junit', importFilePath: 'reports/*.xml', importInfo: info, inputInfoSwitcher: 'fileContent', serverInstance: xrayConnectorId])
    }
    stage('cleanWs') {
        echo "\n\nCleanWs"
        cleanWs()
